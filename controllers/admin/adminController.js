    ...
    try {
        ...
        switch (filter) {
            case 'daily':
                ...
                revenueOrders.revenue = Array.from({ length: 24 }, (_, i) => hourlyData.find(d => d._id === i)?.revenue || 0);
                revenueOrders.orders = Array.from({ length: 24 }, (_, i) => hourlyData.find(d => d._id === i)?.count || 0);
                break;
            case 'monthly':
                ...
                revenueOrders.labels = Array.from({ length: daysInMonth }, (_, i) => `Day ${i + 1}`);
                revenueOrders.revenue = Array.from({ length: daysInMonth }, (_, i) => dailyData.find(d => d._id === i + 1)?.revenue || 0);
                revenueOrders.orders = Array.from({ length: daysInMonth }, (_, i) => dailyData.find(d => d._id === i + 1)?.count || 0);
                break;
            case 'yearly':
                ...
                revenueOrders.labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                revenueOrders.revenue = Array.from({ length: 12 }, (_, i) => monthlyData.find(d => d._id === i + 1)?.revenue || 0);
                revenueOrders.orders = Array.from({ length: 12 }, (_, i) => monthlyData.find(d => d._id === i + 1)?.count || 0);
                break;
        }
        console.log('Revenue Orders Data:', revenueOrders); // Debug output
        // Top Products
        const topProducts = await Order.aggregate([
            { $unwind: '$orderedItems' },
            { $group: { _id: '$orderedItems.product', sales: { $sum: '$orderedItems.quantity' } } },
            { $sort: { sales: -1 } },
            { $limit: 3 },
        ...
        const topCategories = await Order.aggregate([
            ...
            { $unwind: '$product' },
            { $group: { _id: '$product.category', sales: { $sum: '$orderedItems.quantity' } } },
            { $sort: { sales: -1 } },
            { $limit: 2 },
        ...
        const topBrands = await Order.aggregate([
            ...
            { $lookup: { from: 'products', localField: 'orderedItems.product', foreignField: '_id', as: 'product' } },
            { $unwind: '$product' },
            { $group: { _id: '$product.brand', sales: { $sum: '$orderedItems.quantity' } } },
            { $sort: { sales: -1 } },
            { $limit: 4 },
        ...
        const recentOrders = await Order.find()
            ...
            .then(orders => {
                return orders.map(order => ({
                    ...
                    product: order.orderedItems?.length > 0 && order.orderedItems[0].product?.productName
...
const downloadReport = async (req, res) => {
    try {
        ...
        const topProducts = await Order.aggregate([
            { $unwind: '$products' },
            { $group: { _id: '$products.productId', sales: { $sum: '$products.quantity' } } },
            { $sort: { sales: -1 } },
            { $limit: 3 },
            { $lookup: { from: 'products', localField: '_id', foreignField: '_id', as: 'product' } },
            { $unwind: '$product' },
